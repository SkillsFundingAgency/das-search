...
namespace Sfa.Das.Sas.Shared.Components.UnitTests.Mapping.TrainingProvider
{
    ...
    public class GivenClosestLocationResponseMappedToViewModel
    {
        ...
        {
            var result = _sut.Map("123", 12345678, "AB1 3DS", _resultsItemToMap);
        ...
        public void When_Mapping_Then_Items_Are_Mapped()
        {
            ...
            // result.ProviderName.Should().Be(); TODO: Need to pass provider name through
            result.Locations.Count.Should().Be(2);
            result.Locations[0].Distance = 1.23;
            result.Locations[0].AddressWithoutPostCode = "Address 1, Address 2, Town, County";
            result.Locations[0].PostCode = "AB1 2AS";
        }
    }
}
