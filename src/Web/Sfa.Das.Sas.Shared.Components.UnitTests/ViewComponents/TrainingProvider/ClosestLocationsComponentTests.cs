    ...
    public class ClosestLocationsComponentTests : ViewComponentTestsBase
    {
        ...
        public new void Setup()
        {
            ...
            var orchestratorResult = new ClosestLocationsViewModel
            {
                ...
                {
                    new CloseLocationViewModel { Distance = 1.22d, AddressWithoutPostCode = "No 1, The Street, Somewhere", PostCode = "AB12 3FG" },
                    new CloseLocationViewModel { Distance = 3.44d, AddressWithoutPostCode = "", PostCode = "SW12 4ED" }
                }
            };

            _mockTrainingProviderOrchestrator = new Mock<ITrainingProviderOrchestrator>();
            _mockTrainingProviderOrchestrator.Setup(x => x.GetClosestLocations(APPRENTICESHIP_ID, UKPRN, POSTCODE)).ReturnsAsync(orchestratorResult);
        ...
        {
            var result = await _sut.InvokeAsync(APPRENTICESHIP_ID, UKPRN, POSTCODE) as ViewViewComponentResult;
        ...
        public async Task Invoke_ReturnsLocationWithDistance_ForGivenCriteria()
        {
            ...
            var model = result.ViewData.Model as ClosestLocationsViewModel;
            model.Locations[1].Distance = 2.33d;
        }
        ...
        public async Task Invoke_ReturnsLocationWithPostCode_ForGivenCriteria()
        {
            ...
            var model = result.ViewData.Model as ClosestLocationsViewModel;
            model.Locations[1].PostCode = "AB12 3FG";
        }
        ...
        public async Task Invoke_ReturnsLocationAddressCommaDelimitedWitoutPostCode_ForGivenCriteria()
        {
            ...
            var model = result.ViewData.Model as ClosestLocationsViewModel;
            model.Locations[1].AddressWithoutPostCode = "No 1, The Street, Somewhere";
        }
    }
}
