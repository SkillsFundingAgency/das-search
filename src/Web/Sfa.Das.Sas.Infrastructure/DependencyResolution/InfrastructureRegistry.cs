...
namespace Sfa.Das.Sas.Infrastructure.DependencyResolution
{
    ...
    public sealed class InfrastructureRegistry : Registry
    {
        public InfrastructureRegistry()
        {
            ...
            For<IStandardApiClient>().Use<StandardApiClient>().Ctor<string>("baseUri").Is(new ApiSettings().ApprenticeshipApiBaseUrl);
            For<IFrameworkApiClient>().Use<FrameworkApiClient>().Ctor<string>("baseUri").Is(new ApiSettings().ApprenticeshipApiBaseUrl);
            For<IAssessmentOrgsApiClient>().Use<AssessmentOrgsApiClient>().Ctor<string>("baseUri").Is(new ApiSettings().ApprenticeshipApiBaseUrl);
